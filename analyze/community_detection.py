...
def community_detection(config):
    ...
    for com in set(partition_louvain.values()):
        ...
        list_nodes = [
            nodes
            for nodes in partition_louvain.keys()
    ...
    while modularity_newman_new > modularity_newman_old:
        ...
        communities_newman = next(communities_generator)
        modularity_newman_new = nx.algorithms.community.modularity(
            G, communities_newman
        )
        count += 1
    ...
    print("\n")
    print(
        "Partition Girvan-Newman, "
        + str(len(communities_newman_final))
        + " clusters detected: "
    )
    ...
    print("\n")
    print(
        "Modularity score Louvain method: "
        + str(round(nx.algorithms.community.modularity(G, communities_louvain), 2))
    )
    print(
        "Modularity score Girvan-Newman method at level "
        + str(count)
        + ": "
        + str(round(nx.algorithms.community.modularity(G, communities_newman_final), 2))
    )

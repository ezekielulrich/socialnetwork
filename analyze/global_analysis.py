...
def global_analysis(config):
    ...
    in_degree_sequence = sorted(
        [d for n, d in G.in_degree()], reverse=True
    )  # in_degree sequence
    ...
    out_degree_sequence = sorted(
        [d for n, d in G.out_degree()], reverse=True
    )  # out_degree sequence
    out_degreeCount = collections.Counter(out_degree_sequence)
    out_deg, out_cnt = zip(*out_degreeCount.items())
    print(
        "average out_degree %s" % (sum(out_degree_sequence) / len(out_degree_sequence))
    )
    ...
    plt.plot(
        ...
        in_power_law[1],
        "--g",
        label="In degree power law: count = "
        + r"${{{0}}}*degree^{{{1}}}$".format(
            round(in_power_law[2][0], 2), round(in_power_law[2][1], 2)
        ),
    )
    ...
    plt.plot(
        ...
        out_power_law[1],
        "--r",
        label="Out degree power law: count = "
        + r"${{{0}}}*degree^{{{1}}}$".format(
            round(out_power_law[2][0], 2), round(out_power_law[2][1], 2)
        ),
    )
